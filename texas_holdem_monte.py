...
# 몬테카를로 시뮬레이션을 사용하여 승률 계산
def monte_carlo_simulation(hand, board, remaining_deck, num_simulations=10000, num_players=2):
    win_count = 0
    tie_count = 0
    loss_count = 0
    ...
    for _ in range(num_simulations):
        ...
        for _ in range(num_players - 1):
            opponent_hand = random.sample(simulated_deck, 2)
            for card in opponent_hand:
                simulated_deck.remove(card)
            opponent_best_hand = evaluate_hand(opponent_hand + board)
...
win_rate, tie_rate, loss_rate, strongest_opponent_hand, strongest_opponent_cards = monte_carlo_simulation(hand, board,
                                                                                                          deck,
