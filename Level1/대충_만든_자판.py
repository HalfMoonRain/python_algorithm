...
# # 키 뽑아내기
# total_key = ''.join(keymap)
# key = list(set(total_key))
#
# # 키값마다 최대값 설정 (min 사용 해야하므로 일단은 최대값으로 배정)
# times = {i: len(total_key) for i in key}
# for k in key:
#     for i in keymap:
#         for idx, val in enumerate(i):
#             if k == val:
#                 times[k] = min(times[k], idx+1)
#
# result = []
# for t in targets:
#     _sum = 0
...
def solution(keymap, targets):
    ...
    # target 의 값 설정. 만약 없는 글자가 나오면 raise 해서 -1로 값 넣기
    for t in targets:
        _sum = 0
        try:
            for i in t:
