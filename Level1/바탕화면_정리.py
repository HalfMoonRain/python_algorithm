...
드래그의 시작점과 끝점을 담은 정수 배열을 return하는 solution 함수를 작성해 주세요. 드래그의 시작점이 (lux, luy), 끝점이 (rdx, rdy)라면 정수 배열 [lux, luy, rdx, rdy]를 return하면 됩니다.
...
문제 설명의 예시와 같은 예제입니다. (0, 1)에서 (3, 4)로 드래그 하면 모든 파일을 선택할 수 있고 드래그 한 거리는 6이었고,
...
for paperidx, paperval in enumerate(wallpaper):
    for idx, val in enumerate(paperval):
        # 초기값 설정
        if val == '#' and l_min_x == '':
            l_min_x = paperidx
            l_min_y = idx
        ...
        if val == '#' and l_min_x != '':
            l_min_x = min(l_min_x, paperidx)
            l_min_y = min(l_min_y, idx)
            r_max_x = max(r_max_x, paperidx+1)
            r_max_y = max(r_max_y, idx+1)

print([l_min_x, l_min_y, r_max_x, r_max_y])

def solution(wallpaper):
    '''
    좌표로 표시한다음 가장 왼쪽 위에있는 점부터 가장 오른쪽 아래점까지 드래그를 한다.
    '''
    ...
    for paperidx, paperval in enumerate(wallpaper):
        for idx, val in enumerate(paperval):
            ...
            if val == '#' and l_min_x != '':
                ...
                r_max_x = max(r_max_x, paperidx + 1)
                r_max_y = max(r_max_y, idx + 1)
    return [l_min_x, l_min_y, r_max_x, r_max_y]