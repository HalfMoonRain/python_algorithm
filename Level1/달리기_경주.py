...
# Create a dictionary to track the indices
indices = {player: idx for idx, player in enumerate(players)}
...
list 에서 index로 접근하는거는 O(n2)으로 list길이마다 연산이 늘어나지만 dictionary로 만들어두면 값을 추려서 접근할 경우 O(nk)로 속도가 더 개선된다. 
'''
