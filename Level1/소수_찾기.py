...
def eratosthenes_sieve(n):
    ...
    for current_number in range(2, int(n**0.5) + 1):  # n의 제곱근까지만 확인하면 됩니다.
        if sieve[current_number]:  # 현재 수가 소수로 체크되어 있다면
            for multiple in range(current_number**2, n+1, current_number):  # 그 수의 배수는 모두 소수가 아닌 것으로 체크합니다.
                sieve[multiple] = False

    primes = [num for num in range(2, n+1) if sieve[num]]  # True로 남아 있는 수, 즉 소수들을 모아서 반환합니다.
